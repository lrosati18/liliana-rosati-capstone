...
function Form({
  ...
  const handleSubmit = async (e) => {
    ...
    if (editMarkerId) {
      // If it's an edit operation, call fetchMarkers after updating the marker
      await editMarker(editMarkerId);
      fetchMarkers();
    } else {
      // If it's an add operation, call addMarker and fetchMarkers
      await addMarker();
      fetchMarkers(); // No need to pass editMarkerId for add operation
    }
  };
  ...
  const addMarker = async () => {
    ...
    try {
      ...
      await axios.post(`${SERVER_URL}/markers`, markerData, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });

      setMarkerCount(); // Update marker count if needed
  ...
  const editMarker = async (editMarkerId) => {
    const markerData = {
    ...
    try {
      ...
      console.error("Could not update place: ", error);
    }
  };
